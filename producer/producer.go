...
func (p *Producer) SendMessage(ctx context.Context, msg *Message) error {
	...
	pmsg, err := p.config.Converter.ToKafka(p.config, msg)
	if err != nil {
		return err
	}
...
type MessageConverter interface {
	ToKafka(Config, *Message) (*sarama.ProducerMessage, error)
}
...
func (f *messageConverterV1) ToKafka(cfg Config, msg *Message) (*sarama.ProducerMessage, error) {
